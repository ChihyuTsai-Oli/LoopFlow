...
if not exist "%SRC_DIR%\" (
    ...
    echo           LoopFlow\Python\*.py
    echo.
    goto :END_FAIL
)

rem -- Check source has files --
set "SRC_COUNT=0"
for %%F in ("%SRC_DIR%\*") do set /a SRC_COUNT+=1
if %SRC_COUNT%==0 (
...
if exist "%RHINO_SCRIPTS%\" (
    echo [2/3] Removing old version...
    rmdir /s /q "%RHINO_SCRIPTS%"
)
mkdir "%RHINO_SCRIPTS%"
if errorlevel 1 (
...
echo [2/3] Target folder ready: %RHINO_SCRIPTS%

rem -- Copy all scripts --
echo [3/3] Copying scripts...
echo.

robocopy "%SRC_DIR%" "%RHINO_SCRIPTS%" /NJH /NJS /NDL /NP
if errorlevel 8 (
...
set "DST_COUNT=0"
for %%F in ("%RHINO_SCRIPTS%\*") do set /a DST_COUNT+=1
echo   Source : %SRC_DIR%
