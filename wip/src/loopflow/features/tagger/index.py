...
def detail_choice_label(item: dict) -> str:
    ...
    layout = str(item.get("layout") or "").strip() or t("tag_o.032")
    name = str(item.get("dv_name") or "").strip() or t("index.005")
...
def _default_pick_tag(_session: RhinoSession) -> Optional[str]:
    ...
    return pick_block_instance(t("index.007"))
