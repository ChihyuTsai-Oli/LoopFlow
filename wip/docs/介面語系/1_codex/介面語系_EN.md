...
| locale.hint | 之後可用 Document 按鈕右鍵切換（指令 LFLanguage）。 / You can change this later with the Document button's right click (LFLanguage). | You can change this later by right-clicking the Document button (LFLanguage). / 之後可用 Document 按鈕右鍵切換（指令 LFLanguage）。 |
...
| locale.cancelled.first | 已取消選擇語系。尚未記住，下次仍會詢問。 | Language selection canceled. Nothing was saved, so you will be asked again next time. |
| locale.cancelled.switch | 已取消切換語系。 | Language change canceled. |
...
| id | 繁中 | English |
|---|---|---|
| `catalog.001` | 目錄定位點是持久控制物件，建立目錄後請勿刪除；移動目錄時請連同定位點一起移動。 | Catalog anchor points are persistent controls. Do not delete them after creating the drawing index, and move them together with the index. |
| `catalog.002` | 圖名, 圖號 | Drawing name, Drawing number |
| `catalog.003` | 已讀取定位點。 | Anchor points loaded. |
| `catalog.004` | 定位點已配對。 | Anchor points paired. |
| `catalog.005` | 已匯出圖目錄 TXT。 | Drawing-index TXT exported. |
| `catalog.006` | 匯出圖目錄 TXT | Export drawing-index TXT |
| `catalog.007` | 已關閉圖目錄面板。 | Drawing-index panel closed. |
...
| `catalog.010` | 找不到成對的圖號／圖名定位點，已停止，不寫入。 | No matching drawing-number and drawing-name anchors were found. Operation stopped; no changes were made. |
| `catalog.011` | 定位點混入多個圖目錄身分，已停止，不寫入。 | The anchor points belong to multiple drawing indexes. Operation stopped; no changes were made. |
| `catalog.012` | 圖目錄身分不是合法 UUID，已停止，不寫入。 | The drawing-index ID is not a valid UUID. Operation stopped; no changes were made. |
| `catalog.013` | 已取消選取定位點，未寫入。 | Anchor selection canceled. No changes were made. |
| `catalog.014` | 沒有圖目錄定位點可清除。 | There are no drawing-index anchors to clear. |
| `catalog.015` | 已取消清除定位點，未寫入。 | Clear operation canceled. No changes were made. |
| `catalog.016` | 圖號與圖名定位點的逐頁數量不一致，已停止，不寫入。 | The drawing-number and drawing-name anchor counts do not match on every page. Operation stopped; no changes were made. |
...
| `catalog.022` | Rhino 檔尚未儲存，請選擇圖目錄 TXT 的儲存位置。 | This Rhino file has not been saved. Choose where to save the drawing-index TXT file. |
...
| `catalog.039` | 已取消圖目錄寫入。 | Drawing-index update canceled. |
...
| `catalog.042` | 找不到 Eto 介面，無法開啟圖目錄面板。 | Eto UI is unavailable, so the drawing-index panel cannot be opened. |
...
| `catalog.051` | 選取 Layout | Select Layouts |
| `catalog.052` | 生成 圖號/圖名 | Generate drawing numbers and names |
...
| id | 繁中 | English |
|---|---|---|
| `dictionary.001` | Dictionary 含有禁止的 _CB.* 欄，已停止。 | The Dictionary contains prohibited `_CB.*` columns. Operation stopped. |
...
| `dictionary.014` | 第 %s 列 type_id 與第 %s 列重複：%s | The `type_id` on row %s duplicates row %s: %s |
| `dictionary.015` | 第 %s 列 layer_path 與第 %s 列重複。 | The `layer_path` on row %s duplicates row %s. |
| `dictionary.016` | 第 %s 列高程基準不合法：%s | Invalid elevation basis on row %s: %s |
...
| `dictionary.020` | 找不到 Dictionary 檔案 %s。請把字典移回 .3dm 所在的資料夾，或用 Nexus 選單 2 重新指定同資料夾內的 .xlsx。 | Dictionary file %s was not found. Put it back in the same folder as the .3dm file, or use Nexus menu 2 to choose another .xlsx file from that folder. |
...
| `dictionary.033` | Type layer 同步完成（%s）：新建 %s、保留 %s。 | Type Layer sync completed (%s): %s created, %s retained. |
...
| `dictionary.036` | 使用者取消 Type layer 同步。 | Type Layer sync canceled. |
...
| `dictionary.044` | 使用者取消輸入專案名稱。 | Project-name entry canceled. |
| `dictionary.045` | 使用者取消選擇 Dictionary。 | Dictionary selection canceled. |
...
| `document.004` | 已取消選擇語系。尚未記住，下次仍會詢問。 | Language selection canceled. Nothing was saved, so you will be asked again next time. |
| `document.005` | 已取消切換語系。 | Language change canceled. |
| `document.006` | 找不到語系選單介面。 | The language menu is unavailable. |
...
| `extract_cp.019` | 已取消抽出。 | Extraction canceled. |
...
| `extract_cp.026` | 新增一版（保留舊的） | Add new version (keep existing) |
...
| `tag_o.004` | 斷連 | Unlinked |
| `tag_o.005` | 過期 | Out of date |
| `tag_o.006` | 未檢查 | Not checked |
| `tag_o.007` | （未存檔） | (unsaved) |
| `tag_o.008` | 過期會把自動欄改成 ! 並塗橘；斷連改成 ? 並塗紅。Repair 尚未實作。 | Out-of-date Tags show `!` in auto-filled fields and are highlighted orange. Unlinked Tags show `?` and are highlighted red. Repair is not implemented yet. |
| `tag_o.009` | 沒有空間框，略過覆蓋檢查 | No space boundaries; overlap check skipped |
| `tag_o.010` | 過期塗橘寫 !，斷連塗紅寫 ?。未綁定不列出。Repair 尚未實作。 | Out-of-date Tags show `!` in orange; unlinked Tags show `?` in red. Unbound Tags are not listed. Repair is not implemented yet. |
...
| `tag_o.022` | ── 未被 Finish Tag 涵蓋的空間 ── | ── Spaces without a Finish Tag ── |
| `tag_o.023` | 所有空間都有 Finish Tag | Every space has a Finish Tag |
| `tag_o.024` | 斷連：%s。 | Unlinked: %s. |
| `tag_o.025` | 鎖定仍斷連 %s。 | Locked but still unlinked: %s. |
...
| `tag_o.029` | 涵蓋但不判 unbound：%s。 | Covered but not counted as unbound: %s. |
| `tag_o.030` | 沒有掃到可檢查的 Tag | No Tags were found that can be checked |
| `tag_o.031` | 沒有已綁定的 Tag | No bound Tags were found |
| `tag_o.032` | （未命名頁） | (unnamed page) |
| `tag_o.033` | ── 未被 Finish Tag 涵蓋的空間  （%s）── | ── Spaces without a Finish Tag (%s) ── |
...
| `tag_o.036` | 正式 Registry 不在，改用 last-good。 | The current Registry is unavailable. Using last-good instead. |
...
| `infuser.002` | 有些 Height／Finish 是從模型現況讀的，尚未進 Registry。 | Some Height and Finish values were read from the current model and are not yet in the Registry. |
| `infuser.003` | 尚未進 Registry | Not yet in Registry |
...
| `infuser.009` | 目標 Sheet 沒有圖號資料。 | The target Sheet has no drawing number. |
...
| `infuser.023` | 缺目標圖號 | Target drawing number missing |
| `infuser.024` | 目標消失 | Target missing |
| `infuser.025` | 斷連未灌回 | Unlinked; values not updated |
...
| `infuser.030` | 已用目標 Sheet。 | Target Sheet used. |
...
| `infuser.040` | 正式 Registry 不在，改用 last-good revision %s。 | The current Registry is unavailable. Using last-good revision %s. |
...
| `nexus_metadata.004` | 使用者取消 Scan。 | Scan canceled. |
| `nexus_metadata.005` | 局部 Scan 完成，%s 個物件。不得宣告全案可發布。 | Partial scan completed for %s objects. This does not confirm that the full project is ready to publish. |
...
| `nexus_metadata.007` | 使用者取消 Apply。 | Apply canceled. |
| `nexus_metadata.008` | Verify 仍有 %s 項未完成。不可發布。 | Verification still has %s unresolved issues. Publishing is unavailable. |
| `nexus_metadata.009` | 使用者取消 rollback。 | Rollback canceled. |
| `nexus_metadata.010` | 沒有 ID mapping 可還原。 | There is no ID mapping to restore. |
| `nexus_metadata.011` | 已還原 %s 個 object_id。 | Restored %s `object_id` values. |
| `nexus_metadata.012` | mapping 的新 ID 必須是小寫 UUID v4。 | New IDs in the mapping must be lowercase UUID v4 values. |
| `nexus_metadata.013` | Apply 後仍會發生 object_id 碰撞，已停止，不靜默換號。 | Applying this mapping would still create duplicate `object_id` values. Operation stopped; IDs were not changed automatically. |
...
| `nexus_metadata.016` | 此物件同時落在多個空間。請選擇所屬空間： | This object is in more than one space. Choose the space it belongs to: |
| `nexus_metadata.017` | 已 Apply Space／高程。不可發布。 | Space and elevation data applied. Publishing is unavailable. |
| `nexus_metadata.018` | Space／高程 Scan 完成，%s 個物件、%s 個 EXT。不可發布。 | Space/elevation scan completed: %s objects and %s EXT objects. Publishing is unavailable. |
| `nexus_metadata.019` | 使用者取消 Space／高程 Scan。 | Space/elevation scan canceled. |
| `nexus_metadata.020` | 使用者取消 Space／高程 Apply。 | Applying Space/elevation data was canceled. |
| `nexus_metadata.021` | 沒有可寫入的 Space／高程。 | There is no Space/elevation data to write. |
...
| `nexus_metadata.023` | 使用者取消高程框。 | Level-boundary selection canceled. |
| `nexus_metadata.024` | 高程框請選 FFL 或 FL。 | Choose FFL or FL for the level boundary. |
| `nexus_metadata.025` | 高程必須是數字，例如 0 或 320。 | Elevation must be a number, such as 0 or 320. |
...
| `nexus_metadata.028` | 使用者取消 Space Boundary。 | Space Boundary registration canceled. |
...
| `nexus_metadata.030` | 同一個 space_id 出現在多條 boundary，已停止，不靜默換號。 | The same `space_id` appears on multiple boundaries. Operation stopped; IDs were not changed automatically. |
| `nexus_metadata.031` | 平面重疊但樓層不同（已允許）：%s。同樓層請對到同一個樓層框。 | Plan overlap on different levels is allowed: %s. Boundaries on the same level must match the same level boundary. |
| `nexus_metadata.032` | 有 %s 條無效曲線（未封閉或頂點不足）。 | %s curves are invalid because they are open or have too few control points. |
| `nexus_metadata.033` | 高程框類型 | Level boundary type |
| `nexus_metadata.034` | 請選擇高程框類型 | Choose a level boundary type |
| `nexus_metadata.035` | 使用者取消高程框類型。 | Level boundary type selection canceled. |
| `nexus_metadata.036` | 使用者取消選取高程框。 | Level-boundary selection canceled. |
| `nexus_metadata.037` | 高程（例如 0 或 320） | Elevation (for example, 0 or 320) |
| `nexus_metadata.038` | 使用者取消輸入高程。 | Elevation entry canceled. |
| `nexus_metadata.039` | 使用者取消選取空間框。 | Space-boundary selection canceled. |
| `nexus_metadata.040` | 空間名稱 | Space name |
| `nexus_metadata.041` | 使用者取消輸入空間名稱。 | Space-name entry canceled. |
| `nexus_metadata.042` | 有 %s 條無效曲線（未封閉、頂點不足、或缺名稱／樓層）。 | %s curves are invalid because they are open, have too few control points, or are missing a name or level. |
...
| `nexus_metadata.044` | 有 %s 個空間對不到樓層框。空間框須與樓層框高程差在 ±%s 內，且整圈在樓層框裡面。 | %s spaces do not match a level boundary. A space boundary must be within ±%s elevation of a level boundary and entirely inside it. |
...
| `nexus_metadata.056` | 高程顯示 | Display elevation |
...
| `nexus_metadata.072` | 使用者取消檢核。 | Verification canceled. |
| `nexus_metadata.073` | %s 尚未寫入（應為 %s） | %s has not been written (expected %s) |
| `nexus_metadata.074` | %s 現值「%s」不應存在 | %s currently contains “%s” but should not exist |
...
| id | 繁中 | English |
|---|---|---|
| `nexus.001` | 開案檢查完成。可執行 Type Layers、高程／空間框、寫入／檢核 Metadata。匯出字典與發布請用獨立指令。 | Open-project check complete. You can now run Type Layers, register level and space boundaries, and write or verify metadata. Use the separate commands to export the Dictionary or publish. |
...
| `nexus.007` | 沒有可寫入的欄位。 | There are no fields to write. |
| `nexus.008` | 不可發布。 | Publishing is unavailable. |
| `nexus.009` | Scan 完成，%s 個物件。尚未寫入。空間 %s 個 EXT。不可發布。 | Scan completed for %s objects. No data has been written. Spaces: %s EXT. Publishing is unavailable. |
| `nexus.010` | 空間／高程 | Space/elevation |
| `nexus.011` | 已寫入 %s 個物件的 %s。 | Updated %s objects with %s. |
| `nexus.012` | 使用者取消開案檢查。 | Open-project check canceled. |
...
| `nexus.017` | Console 步驟尚未實作：%s | Console step not implemented: %s |
...
| `nexus.026` | 6  檢核模型 Metadata（不寫入） | 6  Verify model metadata (no changes) |
| `nexus.027` | 開案檢查已完成。選一個步驟；Esc 取消。 | Open-project check complete. Choose a step, or press Esc to cancel. |
| `nexus.028` | 取消 | Cancel |
| `nexus.029` | 已完成開案檢查。使用者取消後續步驟。 | Open-project check complete. The remaining steps were canceled. |
...
| id | 繁中 | English |
|---|---|---|
| `registry.001` | 使用者取消發布。 | Publishing canceled. |
| `registry.002` | 局部選取不得發布。請先跑 Nexus 選單 6 檢核通過後再發布。 | A partial selection cannot be published. Run Nexus menu 6 and pass verification before publishing. |
...
| `registry.007` | 沒有 lock 可釋放 | There is no lock to release. |
| `registry.008` | lock 已易主，不刪除。 | The lock is now owned by another process and will not be deleted. |
| `registry.009` | 無法清除過期 lock：%s | Could not remove the stale lock: %s |
| `registry.010` | 無法釋放 lock：%s | Could not release the lock: %s |
| `registry.011` | 無法建立 lock：%s | Could not create the lock: %s |
| `registry.012` | 正式 Registry 檔被佔用（常見是雲端同步還在寫檔）。請等同步結束後再開 Rhino 發一次，不要刪 Project_Registry.json。 | The current Registry file is in use, usually because cloud sync is still writing it. Wait for syncing to finish, then reopen Rhino and publish again. Do not delete `Project_Registry.json`. |
...
| `registry.017` | 正式 Registry 的 project_id 與本次發布不符。 | The current Registry's `project_id` does not match this publication. |
| `registry.018` | pending 寫入後無法重讀：%s | Could not read the pending file after writing it: %s |
| `registry.019` | 發布中斷：%s | Publishing interrupted: %s |
| `registry.020` | 正式 Registry 無法讀取，已停止，未覆寫。%s | The current Registry could not be read. Operation stopped; it was not overwritten. %s |
| `registry.021` | 無法更新 last-good，已停止，未覆寫正式檔。%s | Could not update last-good. Operation stopped; the current Registry was not overwritten. %s |
| `registry.022` | 新內容已寫入 last-good，等同步後再發一次。 | The new data was written to last-good. Publish again after syncing finishes. |
| `registry.023` | atomic replace 失敗，正式檔未刪。%s | Atomic replace failed. The current file was not deleted. %s |
| `registry.024` | 已發布 Registry revision %s，但 last-good 未寫入。 | Published Registry revision %s, but last-good could not be written. |
...
| `registry.042` | EXT 的 level_id 必須為 null。 | EXT `level_id` must be null. |
| `registry.043` | EXT 的 space_display 必須為 EXT。 | EXT `space_display` must be `EXT`. |
| `registry.044` | objects[%s] 必須是 object。 | `objects[%s]` must be an object. |
| `registry.045` | objects[%s] 不得含尺寸／數量欄：%s。 | `objects[%s]` cannot contain size or quantity fields: %s. |
| `registry.046` | objects[%s] 的 object_id 必須是小寫 UUID v4。 | `object_id` in `objects[%s]` must be a lowercase UUID v4. |
| `registry.047` | objects[%s] 的 type_id 不在本 revision 的 types[]。 | `type_id` in `objects[%s]` does not exist in `types[]` for this revision. |
| `registry.048` | objects[%s] 的 space_id 必須是 UUID 或 EXT。 | `space_id` in `objects[%s]` must be a UUID or `EXT`. |
| `registry.049` | spaces[%s] 的 space_id 必須是 UUID 或 EXT。 | `space_id` in `spaces[%s]` must be a UUID or `EXT`. |
...
| `duplicate_layout.004` | 來源 Layout 沒有物件。 | The source Layout contains no objects. |
...
| `duplicate_layout.009` | 已取消複製 Layout。 | Duplicate Layout canceled. |
| `duplicate_layout.010` | 來源 Layout 沒有物件：%s。整批未複製。 | Source Layout %s contains no objects. No copies were created. |
...
| `grab.002` | 選取要綁定的 Tag（Esc 取消） | Select a Tag to bind (Esc to cancel) |
...
| `grab.017` | 已取消 Grab。 | Grab canceled. |
...
| `index.007` | 選取要綁定的 Index Tag（Esc 取消） | Select an Index Tag to bind (Esc to cancel) |
...
| `index.015` | 目標 View 沒有合法的 lf_view_id。請先執行 Anchor Frame。 | The target View does not have a valid `lf_view_id`. Run Anchor Frame first. |
| `index.016` | 請在 Layout 執行 Index。 | Run Index on a Layout. |
| `index.017` | 已取消 Index。 | Index canceled. |
| `index.018` | 沒有 Detail View 可綁定。 | There are no Detail Views to bind. |

## 其他

| id | 繁中 | English |
|---|---|---|
| `other.001` | attr_Lock_不更新>寫入x或X | attr_Lock_No Update > enter x or X |
...
| `laser.003` | 選取要綁定的 Laser Tag（Esc 取消） | Select a Laser Tag to bind (Esc to cancel) |
...
| `laser.010` | 已取消 Laser。 | Laser canceled. |
...
| `laser.012` | View 框沒有合法的固定 transform。請重新執行 Anchor Frame。 | The View frame does not have a valid fixed transform. Run Anchor Frame again. |
...
| `layout_id.006` | 這一頁沒有圖框 | No title block on this page |
| `layout_id.007` | 這一頁有 %s 個圖框，無法決定身分 | This page has %s title blocks, so its identity cannot be determined |
| `layout_id.008` | 沒有可寫入的 Layout 頁。請確認每一頁只有一個已登錄的圖框。 | There are no Layout pages ready to update. Make sure each page has exactly one registered title block. |
| `layout_id.009` | 沒有可寫入的 Layout 頁。 | There are no Layout pages ready to update. |
...
| `layout_id.023` | 已取消 Layout ID，未寫入。 | Layout ID canceled. No changes were made. |
...
| `anchor_frame.013` | 已取消登記 View。 | View registration canceled. |
...
| `paths.014` | Dictionary 須和 .3dm 同一層，不能放在子資料夾。 | The Dictionary must be beside the .3dm file, not in a subfolder. |
...
| `rhino.006` | 封閉框至少需要 3 點 | A closed boundary requires at least 3 points. |
...
| `rhino.014` | 執行中發生例外，已還原 Rhino 狀態。 / %s | An exception occurred during the operation. Rhino state was restored. / %s |
...
| `prompts.002` | 之後可用 Document 按鈕右鍵切換（指令 LFLanguage）。 / You can change this later with the Document button's right click (LFLanguage). | You can change this later by right-clicking the Document button (LFLanguage). |
...
| `prompts.008` | 框選剖面物件與對應的 Text Dot（Esc 取消） | Select the section objects and matching Text Dot (Esc to cancel) |
...
| `prompts.011` | 可按住 Ctrl 或 Shift 一次選多頁。選取列會反白。 | Hold Ctrl or Shift to select multiple pages. Selected rows are highlighted. |
| `prompts.012` | 輸入圖名或圖號搜尋 | Search by drawing name or number |
...
| `prompts.018` | 點擊位置不在任何 Detail 內。 | The clicked point is not inside a Detail. |
...
| id | 繁中 | English |
|---|---|---|
| `runners.001` | 這是 2.0 測試入口「%s」，功能尚未實作（%s）。 | This is the LoopFlow 2.0 test entry point for “%s.” The feature is not implemented yet (%s). |
