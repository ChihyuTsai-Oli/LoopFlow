# LoopFlow 2.0 — 介面語系（Codex 校對稿）

本稿以 Claude 的英文草稿為底稿，由 Codex 依繁中原文校對用字、語意與術語一致性；未動繁中欄。
...
| `catalog.040` | 略過 %s 列。 | Rows skipped: %s. |
...
| `dictionary.038` | 匯出目錄不存在，不建立。 | Export folder does not exist. It will not be created. |
...
| `dictionary.042` | 專案名稱不能空白，也不能含 : \ / * ? " < > \| | Project name cannot be blank, and cannot contain : \ / * ? " < > \| |
...
| `extract_cp.006` | 沒有根圖層名。 | Root layer name is missing. |
...
| `extract_cp.016` | 略過 %s 個已有產出的剖面。 | Skipped sections with existing output: %s. |
...
| `tag_o.011` | 已檢查 %s 個 Tag。 | Tags checked: %s. |
...
| `tag_o.018` | 已掃描 %s 個 Tag | Tags scanned: %s |
...
| `infuser.004` | 已處理 %s 頁 Layout。 | Layout pages processed: %s. |
...
| `infuser.014` | 已注入 %s 個 Tag。 | Tags updated: %s. |
...
| `infuser.037` | 尚未填專案名稱，無法讀 Registry。請先跑 Nexus 選單 2 從字典同步 Type Layers。 | Project name not set yet — cannot read the Registry. Run Nexus menu 2 to sync Type Layers from the Dictionary first. |
...
| `nexus_metadata.015` | 剩餘 %s 項。 | Items remaining: %s. |
...
| `nexus_metadata.032` | 有 %s 條無效曲線（未封閉或頂點不足）。 | Invalid curves (open or too few vertices): %s. |
...
| `nexus_metadata.042` | 有 %s 條無效曲線（未封閉、頂點不足、或缺名稱／樓層）。 | Invalid curves (open, too few vertices, or missing a name or level): %s. |
...
| `nexus.011` | 已寫入 %s 個物件的 %s。 | Updated %s objects: %s. |
...
| `registry.018` | pending 寫入後無法重讀：%s | Could not reopen the pending file after writing it: %s |
...
| `layout_id.007` | 這一頁有 %s 個圖框，無法決定身分 | This page has %s title blocks — cannot determine identity |
...
| `layout_id.015` | 已寫入 %s 頁圖號；新建 %s 個 Sheet 身分，改名 %s 頁。 | Drawing numbers written: %s pages; Sheet IDs created: %s; pages renamed: %s. |
...
| `layout_id.024` | 跳過 %s 頁，詳見報告。 | Pages skipped: %s. See the report for details. |
...
| `data_viewer.003` | 已檢視 %s 個物件。 | Objects viewed: %s. |
...
| `foundation.008` | 無法讀取 JSON：%s | Could not read the JSON: %s |
...
| `paths.012` | 專案名稱不可含 \ / : * ? " < > \|，也不可當資料夾路徑。 | Project name cannot contain \ / : * ? " < > \|, and cannot be used as a folder path. |
...
| `rhino.012` | 還原時找不到 %s 個快照物件，其餘狀態已寫回。 | During restore, %s objects from the snapshot could not be found. All other state was restored. |
