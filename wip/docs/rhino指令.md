...
Layout ID：跑全檔 Layout。系列第一頁寫 `**圖類別__圖編號__圖名`（例如 `**IN__201__立面圖`），後面的頁只寫圖名。核對清單確認才寫入；取消整批零寫入。Layout 頁名改成三欄 `IN__201__立面圖`；圖框 `lf_drawing_no` 寫空格格式 `IN 201`，並寫 `lf_drawing_name`、`lf_sheet_id`。不寫 `lf_scale`、不寫舊 key `DWG_*`。`TAG_ELEV_0` 寫目前頁編號（`201`）。未登錄 Block 勾選真正的圖框（預設全不勾）。只改圖名：改第三欄再跑。要改編號：該頁再加 `**` 再跑。一頁沒有圖框或有兩個圖框則跳過。驗收看 Attribute User Text，圖框上的字要等 D08 改 Block 欄位才會變。不進 Nexus。**尚未 Rhino 8 實機驗證。**
