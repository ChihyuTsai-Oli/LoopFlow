...
Layout ID：跑全檔 Layout。第一次依頁名匯入圖號／圖名，之後以 Sheet metadata 為準。核對清單確認才寫入；取消整批零寫入。只寫已登錄圖框的 `lf_sheet_id`／`lf_drawing_no`／`lf_drawing_name`，不寫 `lf_scale`、不寫舊 key `DWG_*`。`TAG_ELEV_0` 寫目前頁的 `lf_sheet_code`。未登錄 Block 會列出讓使用者勾選真正的圖框（預設全不勾）；沒勾選的不寫。一頁沒有圖框或有兩個圖框則跳過該頁。驗收看 Attribute User Text，圖框上的字要等 D08 改 Block 欄位才會變。不進 Nexus。**尚未 Rhino 8 實機驗證。**
