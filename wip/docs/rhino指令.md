...
TAG-O：跑 `LF_TAG-O`，開 **TAG-O ~ Holy Cargo ~~** 深色面板（可捲動），依 Layout 頁序用顏色列出每一筆斷連（橘：缺來源／過期／歧義；紅：來源不在）。畫面仍是 `-` 的 Tag 會列為過期。並列出沒被 Finish Tag 涵蓋的空間。模型空間也可跑。鎖定的 Tag 仍會判斷並標「鎖定」。門窗與 `TAG_ELEV_0` 沒有來源算正常。家具不因模型裡找不到實例而判為斷連。未知圖塊列為未檢查，不算通過。沒掃到 Tag 時不顯示「全部正常」。**只改面板文字顏色，不改圖面上的 Tag 顏色、不改 UserText**。Repair 尚未實作。**未在 Rhino 8 實機驗證**。
