...
由使用者直接編輯 `資料生態決策表.md` 的「你的決定／說明」欄；ECO／ED／ND 已集中在同一檔。ED-14～16 分別處理家具 `FF-01`、圖框比例與 title-frame 辨識，ED-17／18 處理正式資料化 scope 與 Cabinet 是否阻擋 2.0 首發。AI 讀取修改後，先整理歧義與相依影響，再回寫 `資料生態藍圖.md` 與 `_LoopFlow_命名與資料契約.md`；接著建立現行領域規則清單、10 份 Block manifest fixtures、schema fixtures 與 validator 規格。使用者確認契約前，不開始正式功能程式碼。
