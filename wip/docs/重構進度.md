...
- 狀態：現行工作流程已依 1.0 操作說明、全部 23 支 Python、9 份 Tag 與 1 份圖框重新檢核；文件責任與目錄已整理，尚未修改產品程式碼

## AI 接手入口

本 repo 已建立自足的繁中維護文件。AI 開始前依序讀取根目錄 `AGENTS.md` 與該檔列出的 `wip/docs/` 文件；`資料生態藍圖.md` 負責資料真相邊界，`工作流程模擬.md` 是衝突時的暫定操作基準，最後讀本文件確認即時進度。外部分析檔與已刪除的 `ref/` 不再是必要輸入。
...
- 分支、runtime、路徑與 build 設定：`_LoopFlow_系統設定.md`。
- 操作流程、停留點、golden workflow 與健康檢查：`工作流程模擬.md`。
...
| 2026-08-14 | 決策表複核與字典核對 | 以現行 1.x 程式與 Dropbox 字典逐項驗證使用者填答；補齊 ED-06 的 BH／TH、修正 `TH/BH` 拼寫、ED-09 才改 30.3 cm、ED-13 標明 manual content 刻意例外、ED-14 補 ECO-04 例外與格式警告、ED-07 補 EXT 清單與四種成因 | 字典經 openpyxl 實讀核對；決策表欄數一致；兩份 HTML 重建並通過 `--check` | ED-08 改採固定 local 軸（X→寬、Y→深），否決原先「較大為寬」的動態判定 |
| 2026-08-14 | 建立計量規則工作表 | 在字典檔新增第二個工作表「計量規則」：92 列、含層路徑／ID／名稱／單位／量綱對照，41 筆計數與 2 筆體積已自動填入，49 筆黃底待填並附 8 值下拉選單 | 改檔前備份；改後確認 sheet 0 仍為 14 欄 92 列，1.x 讀取路徑不受影響 | 放第二張表是因為主表新增欄位會被 TagTrigger 寫進每個物件；2.0 需為欄位加 role 才能併回主表 |
| 2026-08-14 | 新增 ND-28 計量規則 | Dictionary 新增「計量規則」欄，詞彙為 `COUNT`／`LEN_W`／`LEN_D`／`LEN_H`／`AREA_WD`／`AREA_WH`／`AREA_DH`／`VOL_WDH`；量綱由 `_08_單位` 決定，不符即阻擋，留空則不計算並列入報告 | 依 92 列實測：41 筆計數與 2 筆體積可推導，49 筆（長度 20、面積 29）需人工填寫 | 欄位尚未在字典建立，各 Type 的實際值待逐筆填寫 |
| 2026-08-14 | 字典事實同步與 `_13` 清理 | 核對 `LoopFlow Dictionary v2.0`：14 欄、92 列、12 類別碼、12 群組；`_CB.01`～`_CB.04` 已移除，`04_CB_櫃體` 材質分類保留；`BH/TH` 退場；ND-06 的七個留白欄已落實。依 ED-10 將 91 列 `我是備註，UCCU` 改為 `(手動輸入備註)` | 改檔前備份，改後重讀驗證：91 列替換、`20_DW` 操作說明保留、結構與版本列不變 | 這是唯一一次修改 Dropbox 工作檔；`20_DW` 說明待 instruction 欄建立後移出 |

## 下一步

ECO-01～11 與 ED-01～18 已全部裁決並回寫；ND-01～28 中只剩 ND-28 的各 Type 實際值待填。下一步是在字典建立「計量規則」欄並逐筆填入 49 個需要人工決定的值，然後回寫 `資料生態藍圖.md` 與 `_LoopFlow_命名與資料契約.md`；接著建立現行領域規則清單、10 份 Block manifest fixtures、schema fixtures 與 validator 規格。使用者確認契約前，不開始正式功能程式碼。
