...
- 每次同步都會詢問「專案名稱」（圖層前綴）。第一次不預填；尚未設定時的標準建議為 `M3D`。確認過一次後，後續會預填上次記住的名稱，仍可人工修改。
...
1. 分別選取一組獨立 Point，作為「圖號」定位格（紅色，圖層 `LoopFlow::Drawing_Number`）與「圖名」定位格（綠色，圖層 `LoopFlow::Drawing_Name`）；兩組共同構成圖目錄的欄位網格。
