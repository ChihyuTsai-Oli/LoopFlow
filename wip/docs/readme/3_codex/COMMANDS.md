...
Opens LoopFlow’s public documentation entry (not the project homepage). It does not change the model and does not guess Rhino / Windows UI language. If the page cannot open, it only explains why. The G03 public-docs pass is not done yet, so the linked guides are still 1.x. This folder is the 2.0 draft that will replace them.
