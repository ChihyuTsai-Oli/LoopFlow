...
- **只發布有 consumer 的欄位**。1.x 的 18 欄裡有 11 欄沒有任何行為 consumer，卻全部寫進 Registry 成為事實上的公開 API；字典移除 `_CB.*` 後剩 14 欄、7 欄無 1.x consumer，其中 `_08`／`_09` 已由 ED-09 取得 2.0 的估算報表 consumer。
