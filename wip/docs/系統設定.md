...
4. 在 Type layer 上放測試用 3D 物件（含一個隱藏、一個鎖定）。選 **4** Scan：命令列應說不寫入、不可發布；封閉方塊應出現「無穩定 local frame」。選 **5** Apply 才寫 ID／Type／空間／高程／尺寸。選 **6** Verify 仍不可發布。
5. 把命令列全文與異常貼回。發布尚未實作，不必測。
