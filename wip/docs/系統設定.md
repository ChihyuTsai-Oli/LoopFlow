...
4  登記空間框
5  Scan（不寫入）
6  Apply（寫入 ID／Type／空間／高程）
7  Verify（再 Scan，仍不可發布）
```
...
2. 選 **2**：指令列會問專案名稱（圖層前綴），第一次預設 `M3D`，之後預設為檔案已記住的名稱，Enter 即可。應出現對應 Type layers 與原點 `DNA_REF_` 點；已有同名 layer 的 UserText 不被蓋掉。圖層應有顯示色，材質名稱為相對路徑（例如 `01_Ceiling_天花::Lighting_Box.燈盒`），顏色與圖層相同。再跑一次不應累加 `DNA_REF_`，舊參考線應被點取代。Esc 取消不應留下半套 layer。
3. 選 **3** 登記樓層框：先選 FFL 或 FL，選封閉曲線後按 Enter，再輸入高程（`0`、`320`）。選 **4** 登記空間框：可複選，輸入「廊道」或「衛浴」。選線時其他物件會被鎖住，結束後還原。房間框須與樓層框高差 ±20 且整圈在樓層框內。不必手填樓層 UUID。
4. 在 Type layer 上放測試用 3D 物件。選 **6** Apply：UserText 應為 `_01_空間名稱`、`_03_ID編號`、`_05_高程基準`、`_07_UUID` 等中文編號 key，**沒有**寬深高或實作數量。物件在房間內應寫入空間名稱而不是 `EXT`。選 **7** Verify 仍不可發布。
