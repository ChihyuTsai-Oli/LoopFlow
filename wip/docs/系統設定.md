...
wip/
  ...
  src/
    loopflow/             # 2.0 套件
      ...
      features/dimension/   # C05 local frame／尺寸／數量；NX-06 已接 Nexus
...
python wip/tests/test_dimension.py
```
...
3. 在 `M3D::_Data::Space_Boundaries` 畫封閉曲線，ObjectName 當空間名稱。**同一樓層的曲線必須用同一個** `lf_level_id`（小寫 UUID）；不同 UUID 會被當成不同樓層。選取曲線後跑指令、選 **3**。共邊應通過；同樓層面積重疊應停止並列出衝突。若平面重疊但樓層不同，會通過並出現警告。牆體等 3D 物件不應被寫入空間欄。
4. 在 Type layer 上放測試用 3D 物件（含一個隱藏、一個鎖定）。選 **4** Scan：尚未寫入、不可發布。**用 Box 畫的封閉方塊**應出現「無穩定 local frame」，這是正確阻擋。選 **5** Apply 會寫 ID／Type 與空間／高程；封閉方塊仍不寫尺寸。選 **6** Verify 仍不可發布。
5. 尺寸請另用「擠出曲線」或圖塊再測一次。物件放到房間曲線內，空間才不會是 `EXT`。Apply 後可在物件內容的 Attribute User Text 看到 `lf_object_id`。發布尚未實作，不必測。
