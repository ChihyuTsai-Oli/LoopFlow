...
4. 在 Type layer 上放測試用 3D 物件（含一個隱藏、一個鎖定）。選 **4** Scan：尚未寫入、不可發布。**長方體封閉方塊**應能量出 W／D／H；挖洞或 L 形才出現「無穩定 local frame」。選 **5** Apply 寫 ID／Type／空間／高程／尺寸。選 **6** Verify 仍不可發布。
5. 物件放到房間曲線內，空間才不會是 `EXT`。Apply 後可在物件內容的 Attribute User Text 看到 `lf_object_id`。發布尚未實作，不必測。
