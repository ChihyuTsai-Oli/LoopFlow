...
wip/
  ...
  src/
    loopflow/             # 2.0 套件
      ...
      features/dimension/   # C05 local frame／尺寸／數量；尚未接 Nexus
...
4. 在 Type layer 上放測試用 3D 物件（含一個隱藏、一個鎖定）。選 **4** Scan：命令列應說不寫入、不可發布；若失敗，例外全文會印在命令列。選 **5** Apply 才寫 ID／Type／空間／高程。選 **6** Verify 仍不可發布。
5. 把命令列全文與異常貼回。尺寸與發布尚未實作，不必測。
