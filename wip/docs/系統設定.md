...
Esc 取消後續步驟。發布尚未實作，清單裡沒有。Apply 後用選 **6** 核對；選 **7** 只在字典目錄新增 `LoopFlow_Dictionary_Export.xlsx`，字體與正式字典相同，有變動的 `diff_status` 會換色。
...
4. 在 Type layer 上放測試用 3D 物件。選 **5** Apply：UserText 應為 `_01_空間名稱`、`_03_ID編號`、`_05_高程基準`、`_07_UUID` 等中文編號 key，**沒有**寬深高或實作數量。物件在房間內應寫入空間名稱而不是 `EXT`。選 **6** Verify：相符會彈窗；不符會彈窗說明並保持那些物件被選取。選 **7** 應在字典目錄出現 `LoopFlow_Dictionary_Export.xlsx`，正式 `LoopFlow_Dictionary.xlsx` 不變；匯出檔字體應與字典相同，缺漏／新增／修改的狀態文字分別為紅／藍／橙。
5. 把命令列與 UserText 異常貼回。發布尚未實作，不必測。
