...
3. 畫封閉曲線當房間框，ObjectName 當空間名稱（「廊道」「衛浴」等中文可用）。**同一樓層的曲線必須用同一個** `_15_樓層ID`（小寫 UUID）。選取曲線後跑指令、選 **3**：曲線會被放到 `M3D::_Data::Space_Boundaries`，之後只有這層上的曲線算 Space。共邊應通過；同樓層面積重疊應停止。
4. 在 Type layer 上放測試用 3D 物件。選 **5** Apply：UserText 應為 `_01_空間名稱`、`_05_寬度W`、`_12_UUID` 等中文編號 key。長方體 Box 應有尺寸；物件在房間內應寫入空間名稱而不是 `EXT`。選 **6** Verify 仍不可發布。
