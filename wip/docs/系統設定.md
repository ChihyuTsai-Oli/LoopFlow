...
3. 畫封閉曲線當房間框，ObjectName 當空間名稱（「廊道」「衛浴」等中文可用）。**同一樓層的曲線必須用同一個** `_10_樓層ID`（小寫 UUID）。選取曲線後跑指令、選 **3**：曲線會被放到 `M3D::_Data::Space_Boundaries`，之後只有這層上的曲線算 Space。共邊應通過；同樓層面積重疊應停止。
