...
2. 選 **2**：應出現 Dictionary 對應的 Type layers 與 `DNA_REF_`；已有同名 layer 的資料不被蓋掉。再跑一次不應累加 `DNA_REF_`。Esc 取消不應留下半套 layer。
3. 在 `M3D::_Data::Space_Boundaries` 畫封閉曲線，ObjectName 當空間名稱，並設物件 UserText `lf_level_id` 為小寫 UUID。選取曲線後跑指令、選 **3**。共邊應通過；面積重疊應停止並列出衝突。牆體等 3D 物件不應被寫入空間欄。
4. 在 Type layer 上放測試用 3D 物件（含一個隱藏或鎖定的）。選 **4** Scan：命令列應說不寫入、不可發布。選 **5** Apply 才寫 ID／Type／空間／高程。選 **6** Verify 仍不可發布。
