...
1  開案檢查（只看狀態，不寫入）
2  同步 Type Layers
3  建立 Space Boundaries（請先選取封閉曲線）
4  Scan（不寫入）
5  Apply（寫入 ID／Type／空間／高程）
6  Verify（再 Scan，仍不可發布）
```

Esc 取消後續步驟。發布尚未實作，清單裡沒有。
...
2. 選 **2**：應出現 Dictionary 對應的 Type layers 與原點 `DNA_REF_` 點；已有同名 layer 的 UserText 不被蓋掉。圖層應有顯示色，材質名稱為相對路徑（例如 `01_Ceiling_天花::Lighting_Box.燈盒`），顏色與圖層相同。再跑一次不應累加 `DNA_REF_`，舊參考線應被點取代。Esc 取消不應留下半套 layer。
3. 先在 `Level_Boundaries_FFL` 或 `FL` 畫樓層外框，ObjectName 寫數字高程（`0`、`320`）。再畫房間框，ObjectName 當空間名稱（「廊道」「衛浴」等中文可用）。房間框與樓層框高程差須在 ±20 內，且整圈在樓層框裡面。選取**房間框**後跑指令、選 **3**：不必手填樓層 UUID；曲線會被放到 `M3D::_Data::Space_Boundaries`。共邊應通過；同樓層面積重疊應停止。
4. 在 Type layer 上放測試用 3D 物件。選 **5** Apply：UserText 應為 `_01_空間名稱`、`_03_ID編號`、`_05_高程基準`、`_07_UUID` 等中文編號 key，**沒有**寬深高或實作數量。物件在房間內應寫入空間名稱而不是 `EXT`。選 **6** Verify 仍不可發布。
...
| `LAYER_LEVEL_FFL` | `M3D::_Data::Level_Boundaries_FFL`；ObjectName 為數字高程基準 |
