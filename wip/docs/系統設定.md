...
Esc 取消後續步驟。發布尚未實作，清單裡沒有。沒有獨立的 Verify；Apply 後用選 **6** 檢查。
...
4. 在 Type layer 上放測試用 3D 物件。選 **5** Apply：UserText 應為 `_01_空間名稱`、`_03_ID編號`、`_05_高程基準`、`_07_UUID` 等中文編號 key，**沒有**寬深高或實作數量。物件在房間內應寫入空間名稱而不是 `EXT`。選 **6** Scan 只檢查、不寫入，仍不可發布。
