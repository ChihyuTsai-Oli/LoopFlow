...
- 建立日期：2026-08-12
- 盤點基準：`releases/LoopFlow/Python/` 全部 23 支 Python；Dropbox 中文版 Dictionary（`LoopFlow Dictionary v2.0`）共 14 欄、92 筆 Type、12 個類別碼與 12 個頂層群組；1.x 的 18 欄中，`_CB.01`～`_CB.04` 已於 2026-08-14 移除
...
上表是現行 14 欄；`_CB.01`～`_CB.04` 已隨 Cabinet／BOM 移出字典，不再是 2.0 的欄位。剩下 14 欄中有 7 欄（`_02`、`_05`～`_09`、`_13`）沒有可驗證的 1.x 行為 consumer——其中 `_08` 與 `_09` 已由 ED-09 取得 2.0 的 consumer（估算報表）。這不表示一律刪除，而表示新 Registry 不能再把所有 UserText 無條件升格為永久公開 API。每個 canonical 欄位都要有 owner、型別、producer、consumer、缺值規則與 migration；其餘放入明確 extension 區。

**欄位還需要一個 role。** 1.x 的 Nexus 以 `for col in df_dict.columns` 無差別寫入（`LF_Nexus.py:397, 496, 775, 803`），所以 Dictionary 主表多一欄就等於每個物件多一個 UserText，也多一個 Registry 欄位。但 Dictionary 裡有兩種本質不同的欄：**物件預設值**（如 `_02_建構狀態`，應該寫進物件並可被 instance 覆寫）與 **Type 層級規則**（如 ND-28 的計量規則，只描述這個 Type 怎麼被計算，寫進物件毫無意義）。2.0 schema 必須讓每欄宣告 role，資料化只寫 `object_default`。這已經不是假設：ND-28 的 `(單位計量規則)` 依使用者裁決放在主表 `_08_單位` 與 `_09_實作數量` 之間（維護時規則就在所依據的單位旁邊），代價是 1.x 會把這個 Type 規則寫進每一個物件、寫進 `DNA_REF_` 參考線，並經 `rs.GetUserText(obj_id)` 進入 Registry；加欄後未重跑 TagTrigger 前，TagChecker 還會因該欄缺值把所有物件判為 error。**這正是 role 機制要解決的第一個實例**：2.0 的資料化只寫 `object_default`，`type_rule` 留在 Type Catalog 內，不下放到物件。
...
原本列在此表的 Cabinet 風險（產物 layer 不受限、被 Nexus 清空 `_CB.*`、方向被排序抹平）**已因使用者裁決離開主鏈**：Cabinet 與 BOM 排除在主工作流程之外並列入後續開發，2.0 主鏈的 Nexus、Registry、Tag 與 Health 都不處理 `_CB.*`，因此不再需要在核心資料鏈解決這組衝突。1.x 事實與程式行號保存在 `工作流程模擬.md` 的「延後工作軌｜Cabinet 與 BOM」，供該工作軌重建時使用。
...
Cabinet 與 BOM 依使用者裁決**不屬於主工作流程**，列入後續開發（1.0 的 BOM 功能過於零碎，混入主鏈會汙染核心資料契約）。主鏈不得因 Cabinet 增加欄位、layer 分支或發布內容；下表的 2.0 責任只在該工作軌啟動時適用。ED-18 已裁決 Cabinet／BOM **不加入 LoopFlow 2.0**，也不是首發門檻；未來若需要，改為單純建模工具或獨立外掛。字典保留的 `04_CB_櫃體` 群組是櫃體的**材質分類**，讓櫃體幾何取得 Type／Space／高程／UUID，與已移除的 `_CB.*` 板件製作資料是兩回事。
