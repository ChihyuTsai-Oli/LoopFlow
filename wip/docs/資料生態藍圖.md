...
- 建立日期：2026-08-12
- 盤點基準：`releases/LoopFlow/Python/` 全部 23 支 Python；Dropbox 中文版 Dictionary（`LoopFlow Dictionary v2.0`）共 15 欄、92 筆 Type、12 個類別碼與 12 個頂層群組；1.x 的 18 欄中，`_CB.01`～`_CB.04` 已於 2026-08-14 移除，同日依 ND-28 新增 `(單位計量規則)` 一欄
...
原始 CodeX 藍圖與 Claude Code 獨立複核已完整整合；過時的 `ref/` 副本已刪除，差異仍可由 Git 歷史追溯。Claude 對仍待裁決事項的獨立意見保留在 `資料生態決策表.md`，不因刪除來源副本而消失。

相關文件：

| 文件 | 負責 |
|---|---|
| `資料生態決策表.md` | 全部待決／已裁決事項與雙 AI 建議；使用者答案的唯一來源 |
| `工作流程模擬.md` | 依 1.0 操作說明、現行 Python 與 10 份 Block 參數重新檢核的 2.0 暫定操作基準 |
| `工作流程模擬.html` | 同內容的深色好讀版；由 Markdown 產生，不手動編輯 |

本文的「現況事實」可由現行程式或 Dictionary 交叉確認，但不等於 2.0 必須沿用；由靜態讀碼推論的 Rhino 行為仍須實機測試。所有尚待使用者確認的原則與實務問題集中在 `資料生態決策表.md`，不在本文件維護第二份答案。若本文的流程敘述與 `工作流程模擬.md` 衝突，操作順序暫以後者為準，疑義回到決策表裁決。
...
| `(單位計量規則)` | Dictionary，2026-08-14 依 ND-28 新增 | 尚無 1.x consumer（新欄） | Type 層級規則，2.0 不應下放到物件；見下方「欄位還需要一個 role」 |
| `_09_實作數量` | 無真正 producer，只補 `-` | 無行為 consumer | 已由 ED-09 定義計算規則，consumer 為 2.0 估算報表 |
...
上表是現行 15 欄；`_CB.01`～`_CB.04` 已隨 Cabinet／BOM 移出字典，`(單位計量規則)` 則依 ND-28 新增。15 欄中有 6 欄（`_02`、`_05`～`_07`、`_13`，以及新欄本身）沒有可驗證的 1.x 行為 consumer——`_08` 與 `_09` 已由 ED-09 取得 2.0 的 consumer（估算報表），`(單位計量規則)` 是 `_09` 計算所依據的規則，本身尚無 1.x consumer 但已是 2.0 estimate pipeline 的必要輸入。這不表示一律刪除，而表示新 Registry 不能再把所有 UserText 無條件升格為永久公開 API。每個 canonical 欄位都要有 owner、型別、producer、consumer、缺值規則與 migration；其餘放入明確 extension 區。

**欄位還需要一個 role。** 1.x 的 Nexus 以 `for col in df_dict.columns` 無差別寫入（`LF_Nexus.py:397, 496, 775, 803`），所以 Dictionary 主表多一欄就等於每個物件多一個 UserText，也多一個 Registry 欄位。但 Dictionary 裡有兩種本質不同的欄：**物件預設值**（如 `_02_建構狀態`，應該寫進物件並可被 instance 覆寫）與 **Type 層級規則**（如 ND-28 的計量規則，只描述這個 Type 怎麼被計算，寫進物件毫無意義）。2.0 schema 必須讓每欄宣告 role，資料化只寫 `object_default`。ND-28 的 `(單位計量規則)` 是第一個實例：它描述「這個 Type 怎麼被計算」，寫進個別物件毫無意義，而且會讓同一份規則出現 N 份可各自漂移的副本——這與 ECO-01「Dictionary 管 Type、物件管 instance」是同一條原則在欄位層級的應用。因此 role 由**欄位語意**決定，不由欄名前綴或擺放位置決定；使用者把它放在 `_08_單位` 旁邊是為了維護時看得見依據，位置不改變它的 role。（1.x 因為無差別寫入所有欄位而做不到這件事，那是舊實作的限制，不是 2.0 的約束。）
...
原本列在此表的 Cabinet 風險（產物 layer 不受限、被 Nexus 清空 `_CB.*`、方向被排序抹平）**已因 ED-18 裁決離開主鏈**：Cabinet 與 BOM 不屬於 LoopFlow 2.0，2.0 主鏈的 Nexus、Registry、Tag 與 Health 都不處理 `_CB.*`，因此不再需要在核心資料鏈解決這組衝突。1.x 事實與程式行號保存在 `工作流程模擬.md` 的「Cabinet 與 BOM 現況記錄」，供它未來若成為獨立工具時參考，不是 2.0 待辦。
...
Cabinet 與 BOM 依 ED-18 裁決**不屬於 LoopFlow 2.0**，也不是首發門檻（1.0 的 BOM 功能過於零碎，混入主鏈會汙染核心資料契約）。主鏈不得因 Cabinet 增加欄位、layer 分支或發布內容。未來若需要，改為單純建模工具或獨立外掛，下表的責任只在該情境下適用，不是 2.0 待辦。字典保留的 `04_CB_櫃體` 群組是櫃體的**材質分類**，讓櫃體幾何取得 Type／Space／高程／UUID，與已移除的 `_CB.*` 板件製作資料是兩回事。

| 現行檔案 | 現行功能 | 必須保留的意圖 | 2.0 建議責任 |
|---|---|---|---|
| `LF_Cabinet_Suite.py` | 產生板件／門片、Shelf／Divider 與 BOM；layer 契約不一致，local direction 被排序丟失，幾何猜測可能覆寫人工更正 | 快速建立可攜帶製作資料的櫃體模型 | **不屬於 2.0**（ED-18）；未來若成為獨立建模工具／外掛，製作資料的所在層與所有權屆時在該專案另定 |
...
- 高程的樓層線搜尋，以及 BH／TH／CH／BC 的判定與顯示；1.x 另有 `TH/BH`（斜線寫法，`LF_Nexus.py:179`）會輸出「較近側／`-`」兩格字串，字典已不再使用，只需 migration 辨識。
...
需要使用者決定的 ECO-01～ECO-11、已有證據的建議與實務語意問題，已拆至 `資料生態決策表.md`。使用者可直接編輯該文件，AI 再依結果回寫本藍圖與下游契約。
