...
本文件把 `資料生態決策表.md` 的建議轉成可實際理解的操作鏈，協助判斷這些原則是否符合工作習慣。兩份流程都已用使用者提供並保存於 repo 的 `loopflow_1.0_workflow_YT.txt` 逐步操作說明、9 份 Tag Block 與 1 份圖框參數重新檢核；CodeX 版以 1.0 操作節奏為骨架，Claude 版保留「在哪個文件執行、失敗停在哪裡、先做哪一段」的視角，兩者與末段比較都保留。

- 決策項目與建議強度：`資料生態決策表.md`（`ECO-*`／`ED-*`／`ND-*` 編號皆指向該表）
- 資料實體、真相邊界與現況盤點：`資料生態藍圖.md`
- 本文件不維護決策答案；使用者的裁決一律寫在決策表，不在此重複。
- **衝突處理**：若藍圖、重構計畫、開發任務表或其他規畫文件對「使用者怎麼操作、在哪裡停留、何時發布／綁定／同步」有不同描述，暫以本文件為操作基準；不要直接覆蓋差異，先把疑義記入決策表再裁決。任務 ID 與工程前置仍由 `開發任務與路徑.md` 負責。
...
> 本檔是依 1.0 實際操作重新檢核的唯一現行流程草案；`工作流程模擬.html` 是由本檔產生的深色閱讀版。先前的初版提案已刪除。
...
| Cabinet 與三個 2D Generator 不屬於強制主鏈 | 混在建模步驟，界線不清 | 依 ED-18 裁決，Cabinet／BOM 不屬於 LoopFlow 2.0；2D Generator 列為可隨時使用的獨立工具 |
...
**Cabinet 不在此階段**：`LF_Cabinet_Suite` 與 BOM 依 ED-18 不屬於 LoopFlow 2.0。櫃體幾何仍可用 Rhino 一般工具建立；1.x Cabinet 的行為與衝突記錄集中在後面的「Cabinet 與 BOM 現況記錄」。
...
**2.0 建議** → Boundary 保存穩定 `space_id`、顯示名稱與必要的 level／priority。ED-07 最終裁決維持 `EXT` 不拆成多值，改以覆蓋率報告列出被判為 `EXT` 的物件與四種成因（無 boundary 圖層／圖層無邊界／取不到 bbox／有邊界但未命中），由使用者目視確認。
...
## Cabinet 與 BOM 現況記錄（不屬 LoopFlow 2.0）

使用者已裁決：**`LF_Cabinet_Suite` 與 BOM 不屬於 LoopFlow 2.0**，也不是首發門檻（ED-18）。未來若仍需要，改為單純建模工具或獨立外掛，屆時另建契約；不是本 repo 目前規劃的重建對象。 理由是 1.0 的 BOM 功能過於零碎，混入主鏈只會汙染核心資料契約。因此：
...
- 若未來成為獨立建模工具／外掛，製作資料要放在哪一層、由誰擁有，以及是否需要獨立的 BOM 消費端，屆時在該專案另行決定。
...
前三項在 1.x 構成三方衝突（產生不限 layer、BOM 限 `04_CB`、Nexus 依 layer 清空）。既然 Cabinet 已移出主鏈，2.0 的處理方式是**讓 Nexus 不再碰 `_CB.*`**，衝突隨之消失；`_CB.01`～`_CB.04` 已於 2026-08-14 從字典移除，如何保存製作資料留給日後的獨立建模工具／外掛決定。
...
| Cabinet Suite 產物**可在任何 layer**；只有 BOM Update 需要 `04_CB` | 寫成「產生前先確認目標 layer」，把限制講得比實際嚴格 | 已由使用者裁決取代：Cabinet／BOM 不屬於 2.0，事實改記於「Cabinet 與 BOM 現況記錄」 |
...
- **Cabinet layer**：1.0 錄影說 Suite 產物可在任何 layer，這是對的——`run_cabinet_gen` 產生時不檢查 layer。但 `LF_Nexus > TagTrigger` 會把非 `04_CB` layer 上物件的 `_CB.*` 全部清成 `-`。所以「任何 layer 都可以」只在**跑 TagTrigger 之前**成立。這是錄影說明、BOM Update 限制與 Nexus 行為三者之間的真實衝突，不是錄影講錯。**使用者其後裁決 Cabinet／BOM 不屬於 2.0**，所以這項衝突不需要解決；完整記錄見「Cabinet 與 BOM 現況記錄」。
...
- `DNA_REF_` 參考線依 ED-11 保留，用途是讓每個 Dictionary 圖層在場景中至少有一個物件、避免空圖層被 Purge。重跑必須依穩定 Type ID 更新或取代既有參考線，不得每次累積 92 條。它們是 curve，不在資料化的掃描型別內（見階段 5），所以不會變成 Registry 裡的模型物件。
...
**Cabinet 不屬於 LoopFlow 2.0**：依 ED-18 裁決，`LF_Cabinet_Suite` 與 BOM 不進 2.0。這階段的櫃體就是一般 3D 幾何，適用與其他物件相同的 Type、Space、高程與 ID 規則；2.0 的 Nexus 不再處理 `_CB.*`，也不因 Cabinet 增加 layer 判定分支。1.x 的 Cabinet 行為與三方衝突記錄在「Cabinet 與 BOM 現況記錄」，只作為它未來若成為獨立工具時的參考輸入，不是 2.0 待辦。
...
**2.0 建議**：Boundary 保存穩定 `space_id`、顯示名稱與必要的 level／priority。現況「未命中」與「室外」一律寫 `EXT`，導致 TAG-O 的空間覆蓋檢查無法分辨「這東西在室外」與「這東西漏掉了」；ED-07 最終裁決不拆成多值，改以覆蓋率報告列出 `EXT` 物件與四種成因，由使用者目視確認。
...
| 空間未命中一律寫 `EXT` | 維持 `EXT` 不拆分；覆蓋率報告列出 `EXT` 物件與四種成因，供人工確認 | ED-07 |
...
- **只發布有 consumer 的欄位**。1.x 的 18 欄裡有 11 欄沒有任何行為 consumer，卻全部寫進 Registry 成為事實上的公開 API；字典移除 `_CB.*`、新增 `(單位計量規則)` 後現為 15 欄，6 欄無 1.x consumer，其中 `_08`／`_09` 已由 ED-09 取得 2.0 的估算報表 consumer。
...
- 三個 2D Generator 重建；Cabinet／BOM 不屬於 2.0（ED-18），不在此列

**不阻擋主鏈的獨立工具**：`LF_2D_DW_Gen`（8 種門、3 種窗）、`LF_2D_Cabinet_Gen`（高櫃／下櫃／衣櫃）、`LF_2D_Shelf_Gap`（等距層板）。依錄影說明它們沒有流程相依，可隨時使用，1.x 版本可以一直用到最後。`LF_Cabinet_Suite` 與 BOM 則依 ED-18 不屬於 LoopFlow 2.0；主鏈的 Nexus、Registry、Tag 與 Health 都不處理 `_CB.*`。

**建議最先完成的核心使用者功能工作軌**：階段 5 的 Scan／Apply／再驗證。它同時解掉 UUID 不可逆損失（ECO-11）、BC 靜默錯誤（ED-06）與空間未命中歧義（ED-07）。這不代表跳過資料契約與最小 foundation；工程前置仍依 `開發任務與路徑.md`，但契約與骨架完成後，不應先被其他業務功能插隊。
