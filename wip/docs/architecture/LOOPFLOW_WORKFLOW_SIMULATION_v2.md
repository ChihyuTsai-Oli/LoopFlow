...
`wip/docs/loopflow_1.0_workflow_YT.txt` 是**操作邏輯的參考**，用來確認 1.0 實際怎麼被使用、使用者在哪些點介入。使用者已確認：這套流程在 1.0 使用上有一些限制，但沒有造成大問題，而且**流程本身不是不可更動的**——2.0 應該配合新的程式架構重新思考步驟怎麼排。

因此本文出現的「補回」「必須保留」「不能刪掉」一律指**意圖層級**，不是要求 2.0 照抄 1.0 的按鈕順序：

- **要保留的是控制權**：何時寫入模型資料、何時發布、切哪張圖、抽哪些線、Tag 綁誰、只同步當前頁或全部頁，這些決定仍屬使用者。
- **可以重新設計的是形式**：階段怎麼合併或拆分、指令怎麼命名、哪些檢查自動串接、哪些介面取代 1.0 的脆弱做法（例如 Anchor Frame → View Registration）。
- **不可無意遺失的是事實**：本文記錄的 1.0 行為、欄位所有權與已知衝突，是重新設計時的輸入；要改掉某項行為可以，但要是有意識的裁決，不是搬程式時順手改掉。

換句話說，本文是「現況 + 已知風險 + 建議方向」，不是 2.0 的流程契約。與新架構衝突時，以新架構的論證為準，並把差異回寫決策表。

> 本檔是依 1.0 實際操作重新檢核的現行流程草案；`LOOPFLOW_WORKFLOW_SIMULATION_v2.html` 是由本檔產生的深色閱讀版。未加 `_v2` 的舊 MD／HTML 已標示為初版提案，只供差異追溯。
