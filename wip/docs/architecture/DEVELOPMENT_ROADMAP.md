...
| LF-D01 | Tagger Grab／`tagger-grab` | A04、B03、C01–C03 | Grab feature、入口與 fixtures | Rhino 狀態復原、重複執行與取消通過 |
...
| LF-D04 | Tagger Layout ID／`tagger-layout-id` | D01、Layout 契約 | Layout ID feature、入口與 fixtures | Layout／Tag identity 一致 |
| LF-D05 | TAG-O／`tag-o` | D01–D04 | TAG-O feature、入口與 fixtures | Tag consumer 行為及錯誤路徑通過 |
| LF-D06 | Infuser Part／`infuser-part` | D01–D05、C03 | Part feature、入口與 fixtures | 部分更新、last good 與警告狀態通過 |
...
| LF-G02 | Build／Installer／RHC／`build-release` | 核心功能與 migration 完成 | payload、ZIP、manifest、SHA-256、RHC、可回復安裝器 | 全新／升級／重複安裝與 rollback 通過 |
