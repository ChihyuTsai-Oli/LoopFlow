...
**前期評估階段，還沒有計算程式。** 2026-09-01 已在實際 worksession 模型上完成資料盤點與量測方法驗證。櫃體另有限定流程：由 Toolkit 建立帶 `QTY_CB_` 前綴的真正 Rhino Group Name，QTY 只讀該群組並輸出 Z-up OBB 原始長寬高（2026-09-03 已在 0°／30° 櫃體再次驗證）。2026-09-02 測檔已補 `_LoopFlow_Config`（2.0.7：`loopflow/` 含 Registry revision 4，並預留空的 `loopflow_QTY/`）；細節見 [`docs/前期評估/資料盤點.md`](./docs/前期評估/資料盤點.md) §9。
