...
此資料夾是**已發布**的 LoopFlow 2.0 出圖產品：原始碼、測試與實作文件。對外以 Package Manager `.yak` 從 `main` 安裝。`v1/` 與 `v1.0.0` 是 1.x 唯讀參考。數量計算是同 repo 的另一個產品，在 `qty/`，不在這裡。
...
v2/
  ...
  packaging/        # G02 安裝套件來源；g02-spike 登錄全部正式指令並附 LoopFlow.rui
  tests/            # 自動測試
...
`docs/工作流程.html` 是由 `docs/工作流程.md` 產生的閱讀版，不是第二份規格；修改 Markdown 後執行 `v2/tools/build_workflow_html.py` 更新。
