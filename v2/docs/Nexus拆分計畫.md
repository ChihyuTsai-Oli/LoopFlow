...
**做**：每一次同步都彈出視窗問專案名稱（第一次不預填 `M3D`，之後預填已記住的 `lf_layer_prefix`）；Dictionary 有、Rhino 無的 layer 建立並帶 `construction_default`；已有同名 layer 保留既有 UserText；依圖層代號前綴套用顯示色；新建材質顏色與圖層相同，已存在的同名材質只掛接不改色；`DNA_REF_` 為原點點物件，依 `type_id` 更新不累積；`20_DW` 排除 child layer；獨立指令 `LF_Export_Type_Layers` 在字典目錄匯出獨立 XLSX，不覆寫正式檔。  
...
| A06 | **已完成**；Dictionary／UUID／Space／local frame／Registry 形狀已在 `v2/fixtures/contract/`；各 NX 包再補該包的 Scan 報告案例 |
