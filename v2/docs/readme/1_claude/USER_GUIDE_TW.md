...
2. **字典灌入資料**：Excel 字典定義每種材質／工項的預設值，同步成 Rhino 圖層；模型建好後，把物件放進對應圖層，就能把這些資料寫入每個物件（ID、材質、高程、空間…），再發布成一份公開的資料快照。
3. **銜接 2D**：2D 端不重新建模，而是讀取這份發布出來的資料快照，透過 Tag 顯示出來。

這條鏈是單向資料流：**3D 決定資料的內容，2D 只負責把資料呈現在圖紙上**。
...
- **分界線之後**：都是 2D 世界——剖面產生後，接下來的圖紙編號、Tag 綁定、資料顯示，做的是「怎麼把已經存在的資料整理成圖面」，不是重新建立幾何或重新定義資料。
