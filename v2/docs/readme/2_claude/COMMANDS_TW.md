...
| 字典 | `LFOpenDictionaryExport` | 開啟從模型匯出的字典，供人工核對 |
...
**輔助工具**：[A1 雙機同步](#a1-雙機同步) · [A2 資料檢視](#a2-資料檢視) · [A3 擷取線稿](#a3-擷取線稿) · [A4 複製 Layout](#a4-複製-layout) · [A5 使用說明](#a5-使用說明)
...
用系統預設的 Excel 開啟這份 `.3dm` 記住的正式字典檔（預設檔名 `LoopFlow_Dictionary.xlsx`）。只負責開檔，不編輯、不建立檔案。
...
- 第一列是紅字提示：這份檔案不能當正式字典開啟，也不可以覆蓋正式檔。
- 欄位跟正式字典一樣的 15 欄，多一欄 `diff_status`，用顏色標示差異：
  - **紅字**：正式字典有、模型裡卻不見的圖層（`missing_in_rhino`）。
  - **藍字**：模型裡新增、字典還沒有的圖層（`added_in_rhino`）。放在表格最下面；併回正式字典時記得給一個**還沒用過的** `_03_ID編號`，不要沿用舊編號。
...
自動確認工作檔路徑、字典、`project_id`、資料版本、Rhino 文件單位是否齊全。

- 找不到必要設定時會停止，不會亂猜路徑或自己建立檔案。
...
- 第一次會彈窗問「專案名稱」（圖層前綴，預設 `M3D`），之後每次都會預填記住的名稱，直接按確定即可，也可以改名。
- 第一次會跳出選 Excel 檔的視窗，選定後記住檔名，之後不會再跳出來（除非那個檔名被改掉找不到）。
- 字典有、Rhino 沒有的圖層：自動建立，並帶入預設值。
- Rhino 已經有的同名圖層：**保留既有內容**，不會被字典覆蓋。
...
把每個物件的 ID、材質對照、所在空間、高程、資料版次寫進物件屬性。如果一個物件同時落在兩個空間框裡，會依序把畫面拉近該物件，再彈出清單讓使用者選一個所屬空間。
...
把模型資料打包成一份新的 Registry（提供給 2D 端使用的唯讀資料快照）：
...
3. 系統用 Text Dot 的文字去找同名的 Clipping Plane：完整名稱相同才算數（不分大小寫）；例如 `LF_立面` 不會誤配到 `LF_立面2`。找不到，或同時對到兩個以上，都會停止、不猜測。
...
4. 寫入內容：Layout 頁名整理成三欄、圖框寫入圖號與圖名、文件裡建立 Sheet metadata。**比例（`lf_scale`）由人工自己填，這個指令不會動它。**
...
只讀取 Sheet metadata 來產生圖目錄文字，不會自己重新解析頁面內容。開啟面板後：

1. 分別選一個獨立的 Point 當作「圖號」定位點（紅色，圖層 `LoopFlow::Drawing_Number`）與「圖名」定位點（綠色，圖層 `LoopFlow::Drawing_Name`）。
2. 在清單裡勾選要列入的 Sheet（依頁序／圖號／圖名／頁名四欄顯示，可用 Shift 連選、Ctrl 加選）。
3. **Build**：預覽後在定位點左下角產生目錄文字（`LoopFlow::Drawing_Text` 圖層）。
4. **Refresh**：只更新文字內容，**已經被使用者移動過的文字會留在原地**，不會被拉回定位點。

定位點只記住「這一格對應哪個 Sheet」，不保存圖號、圖名的實際內容，也不保存頁名，所以定位點的數量可以比實際 Sheet 多（例如 40 格只用 28 張圖）。「清除定位點並還原圖層」會清掉目錄資料、刪除產生的文字，並把定位點放回選取前的原圖層。
...
3. 系統以框內剖面線稿現況的中心點當 2D 原點，沿著登記時寫死的方向射線找物件；同一個物件只算一次，如果射線同時打到兩個以上物件，會停止並列出候選清單（只顯示圖層名稱，不顯示物件 ID），讓使用者自己選。

**不會寫入的情況**：按 Esc、點在 Detail 外面、Tag 被鎖定、選錯圖塊種類（Grab／Index／`TAG_DW`／圖框）、同時對到 0 個或 2 個以上的登記框、射線沒打到任何東西、來源物件沒有可用的識別碼。
...
- Height／Finish 類的 Tag 會綁定物件的識別碼。
- 家具（Item）類的 Tag 會綁定**圖塊名稱**（例如 `FF-01__Chair-1`）**以及該實例本身**——這樣之後改圖塊名稱或刪除該實例，[12/13 注入資料](#1213-注入資料) 與 [14 健康檢查](#14-健康檢查tag-o) 才能跟得上變化。同款家具可以共用同一個圖塊名稱。
...
> 提醒：`TAG_ELEV_0`（六方向索引牌）不是這支指令綁定的對象，它的內容全部手動填寫。
...
- **只做 Refresh，不會自動 Attach 或 Detach** 任何參照，也不會改動 `.rws` 設定檔本身。
...
把 Rhino Section 產生的剖面線稿（圖 A，持續與 3D 同步、不能編輯）複製成一份可以自由編修的獨立圖面（圖 B）。
...
- 圖框的圖號、圖名會先清空（等重新跑 07 才會填上），但比例（`lf_scale`）保留。
- 新頁名保留三欄格式並加上 `_CopyN`，但不會自動加 `**`／`//`，避免被誤認成新的編號系列。

取消、來源沒有物件、或執行失敗，都不會留下複製到一半的頁面。

## A5　使用說明

**指令**：`LFDocument`

開啟這份文件的正式入口網頁（不是專案首頁）。不會修改模型，也不會猜測系統語系；打不開時只會說明原因。
