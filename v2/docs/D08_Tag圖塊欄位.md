...
1. **不要改** Git 裡的 `v1/Tag_Blocks.3dm`（那是 1.x 發布檔）。
2. 2.0 官方檔是 `v2/docs/Tag_Blocks_3dm/Tag_Blocks.3dm`（一份，檔內中英說明並列）。
...
從未輸入過 `x` 的實例，鎖定 key 可能不會出現在 UserText 列表（畫面上仍可看到 `x to lock`）。這是 Rhino 行為，不是壞掉；要列表上有這一欄，插入新圖塊或手動加 `lf_00_lock_state` 即可。門窗與圖框本來就沒有鎖定欄。

1. 仍用 `v2/tools/擷取tag_block文字.py` 選各圖塊實例，確認公式已是 `lf_*`、提示已是英文，且沒有 `DWG_NO`、`DWG_NAME`、`Category`、`attr_mat_*`、`attr_Lock_不更新>寫入x或X` 等舊名。
