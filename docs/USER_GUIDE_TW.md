...
2. **字典灌入資料**：Excel 字典定義每種材質／工項的預設值，同步成 Rhino 圖層；模型建好後，把物件放進對應圖層，就能把這些資料寫入每個物件（ID、材質、高程、空間…）。`00_STR` 結構層與其子層上的物件不寫入物件資料，也不當 Tag 來源。
...
官方 Dictionary 範本與 `Tag_Blocks.3dm` 隨安裝包提供：套件目錄留一份，執行產品指令時複製到「文件\LoopFlow」。這個預設資料夾不見時，再跑指令會拷回來。若安裝版號變了，該資料夾裡的官方檔會換成與套件相同的版本（這次有拷才開資料夾）。多出來的檔不會刪。那是方便找、方便再複製到各專案的來源，不是專案工作資料夾。
