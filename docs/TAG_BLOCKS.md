...
>
> 2.0 keeps **one** `Tag_Blocks.3dm`. The file has Chinese and English notes together; there are not two 3dm files. Fixed labels on the block (`Grab`, `Laser`, `W.`, `H.`) are drawn text, not data fields. Formula default prompts stay in English and do not follow `LFLanguage`. After install, the library is copied to `Documents\LoopFlow`. If that folder is missing, a formal command copies it back. Installing a new version replaces the official file there so it matches the package.
>
